...
import { streamJSONLLines } from '@/common/helpers/data-reader'

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type AnyLine = Record<string, any>

export async function parseSessionPlayback(
  jsonlPath: string,
  sessionId: string,
): Promise<PlaybackData> {
  const lines: AnyLine[] = []
  await streamJSONLLines(jsonlPath, line => lines.push(line))
  ...
  for (const l of lines) {
    ...
    if (l.type === 'user') {
      ...
      let text = ''
      const tool_results: SessionTurn['tool_results'] = []
