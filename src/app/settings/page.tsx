...
function JsonValue({ value, depth = 0 }: { value: unknown; depth?: number }) {
  ...
  if (typeof value === 'boolean') return <span className="text-[#b45309] dark:text-[#fbbf24]">{String(value)}</span>
  if (typeof value === 'number') return <span className="text-[#047857] dark:text-[#6ee7b7]">{value}</span>
  if (typeof value === 'string') return <span className="text-[#c2410c] dark:text-[#f9a875]">&quot;{value}&quot;</span>
                  ...
                  {data.plugins.map((plugin, i) => (
                    <div key={i} className="border border-border rounded p-3 flex items-center justify-between gap-4">
                      ...
                      <div className="text-right shrink-0">
                        <span className="text-[#047857] dark:text-[#6ee7b7] font-mono text-xs">v{plugin.version}</span>
